...
public class AodvEvaluationUnit extends EvaluationUnit{
	...
	public void evaluateSpeedAnalysis() {
		...
		for(int i=2; i<MAX_NETWORK_WIDTH+1; i++){
			...
			transmissionTime_max[1][i-2] = aodvSimulator.speedAnalysis(i, (int)Math.pow(i, 2)-i, (int)Math.pow(i, 2)-1);
		...
		try {
			ChartUtilities.saveChartAsPNG(new File("Output/AODV/AODV_Uebertragungszeit.png"),chart,CHART_WIDTH,CHART_HIGHT);
		} catch (IOException e) {
		...
		try {
			ChartUtilities.saveChartAsPNG(new File("Output/AODV/AODV_Uebertragungszeit_Kollisionen.png"),chart2,CHART_WIDTH,CHART_HIGHT);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
