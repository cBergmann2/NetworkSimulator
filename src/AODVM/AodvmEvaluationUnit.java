...
public class AodvmEvaluationUnit extends EvaluationUnit {
	...
	public void evaluateSpeedAnalysis() {
		...
		for (int i = 0; i < networkWidth.length; i++) {
			...
			transmissionTime_max[1][i] = aodvmSimulator.speedAnalysis(networkWidth[i], 0,
					(int) Math.pow(networkWidth[i], 2) - 1) / 1000.0;
			transmissionTime_max_collisions[0][i] = numberOfNodes[i];
			transmissionTime_max_collisions[1][i] = aodvmSimulator.getCollisions();
			transmissionTime_msg_max[0][i] = numberOfNodes[i];
			transmissionTime_msg_max[1][i] = aodvmSimulator.getMsgTransmissionTime() / 1000.0;
			System.out.println("Max Simulation for " + Math.pow(networkWidth[i], 2) + " nodes completed.");

			transmissionTime_min[0][i] = numberOfNodes[i];
			transmissionTime_min[1][i] = aodvmSimulator.speedAnalysis(networkWidth[i], 0, 1) / 1000.0;
			transmissionTime_min_collisions[0][i] = numberOfNodes[i];
			transmissionTime_min_collisions[1][i] = aodvmSimulator.getCollisions();
			transmissionTime_msg_min[0][i] = numberOfNodes[i];
			transmissionTime_msg_min[1][i] = aodvmSimulator.getMsgTransmissionTime() / 1000.0;
			System.out.println("Min Simulation for " + Math.pow(networkWidth[i], 2) + " nodes completed.");

			transmissionTime_med[0][i] = numberOfNodes[i];
			transmissionTime_med[1][i] = aodvmSimulator.speedAnalysis(networkWidth[i], 0,
					(networkWidth[i] / 2) * networkWidth[i] + networkWidth[i] / 2) / 1000.0;
			transmissionTime_med_collisions[0][i] = numberOfNodes[i];
			transmissionTime_med_collisions[1][i] = aodvmSimulator.getCollisions();
			transmissionTime_msg_med[0][i] = numberOfNodes[i];
			transmissionTime_msg_med[1][i] = aodvmSimulator.getMsgTransmissionTime() / 1000.0;
		...
		try {
			ChartUtilities.saveChartAsPNG(new File("Output/AODVM/AODVM_Uebertragungszeit.png"), chart, CHART_WIDTH,
					CHART_HIGHT);
		} catch (IOException e) {
		...
		try {
			ChartUtilities.saveChartAsPNG(new File("Output/AODVM/AODVM_Uebertragungszeit_Kollisionen.png"), chart2,
		...
		try {
			ChartUtilities.saveChartAsPNG(new File("Output/AODVM/AODVM_Uebertragungszeit_Nachricht.png"), chart2,
					CHART_WIDTH, CHART_HIGHT);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public void evaluateCostAnalysis() {
		...
		for (int i = 0; i < networkWidth.length; i++) {
			...
			maxDistance[1][i] = aodvmSimulator.energyCostAnalysis(networkWidth[i],
					(int) Math.pow(networkWidth[i], 2) - networkWidth[i], (int) Math.pow(networkWidth[i], 2) - 1);

			maxDistance_collisions[0][i] = numberOfNodes[i];
			maxDistance_collisions[1][i] = aodvmSimulator.getCollisions();
			maxDistance_onlyTransmissionEnergy[0][i] = numberOfNodes[i];
			maxDistance_onlyTransmissionEnergy[1][i] = aodvmSimulator.getConsumedEnergyInReciveMode()
					+ aodvmSimulator.getConsumedEnergyInTransmissionMode();
			networkLifetime = aodvmSimulator.getNetworkLifetime();
			maxDistance_onlyPayloadMsg[0][i] = numberOfNodes[i];
			maxDistance_onlyPayloadMsg[1][i] = aodvmSimulator.energyCostAnalysisWithoutRDP(networkWidth[i],
					(int) Math.pow(networkWidth[i], 2) - networkWidth[i], (int) Math.pow(networkWidth[i], 2) - 1);
			...
			minDistance[1][i] = aodvmSimulator.energyCostAnalysis(networkWidth[i], 0, 1);
			minDistance_collisions[0][i] = numberOfNodes[i];
			minDistance_collisions[1][i] = aodvmSimulator.getCollisions();
			minDistance_onlyTransmissionEnergy[0][i] = numberOfNodes[i];
			minDistance_onlyTransmissionEnergy[1][i] = aodvmSimulator.getConsumedEnergyInReciveMode()
					+ aodvmSimulator.getConsumedEnergyInTransmissionMode();
			networkLifetime = aodvmSimulator.getNetworkLifetime();
			minDistance_onlyPayloadMsg[0][i] = numberOfNodes[i];
			minDistance_onlyPayloadMsg[1][i] = aodvmSimulator.energyCostAnalysisWithoutRDP(networkWidth[i], 0, 1);
			...
			medDistance[1][i] = aodvmSimulator.energyCostAnalysis(networkWidth[i], 0,
					(networkWidth[i] / 2) * networkWidth[i] + networkWidth[i] / 2);
			medDistance_collisions[0][i] = numberOfNodes[i];
			medDistance_collisions[1][i] = aodvmSimulator.getCollisions();
			medDistance_onlyTransmissionEnergy[0][i] = numberOfNodes[i];
			medDistance_onlyTransmissionEnergy[1][i] = aodvmSimulator.getConsumedEnergyInReciveMode()
					+ aodvmSimulator.getConsumedEnergyInTransmissionMode();
			networkLifetime = aodvmSimulator.getNetworkLifetime();
			medDistance_onlyPayloadMsg[0][i] = numberOfNodes[i];
			medDistance_onlyPayloadMsg[1][i] = aodvmSimulator.energyCostAnalysisWithoutRDP(networkWidth[i], 0,
					(networkWidth[i] / 2) * networkWidth[i] + networkWidth[i] / 2);
		...
		try {
			ChartUtilities.saveChartAsPNG(new File("Output/AODVM/AODVM_Umgesetzte_Energie.png"), chart, CHART_WIDTH,
					CHART_HIGHT);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		try {
			ChartUtilities.saveChartAsPNG(
					new File("Output/AODVM/AODVM_Umgesetzte_Energie_nur_Uebertragungsenergie.png"), chart3, CHART_WIDTH,
					CHART_HIGHT);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		try {
			ChartUtilities.saveChartAsPNG(new File("Output/AODVM/AODVM_Umgesetzte_Energie_Kollisionen.png"), chart2,
					CHART_WIDTH, CHART_HIGHT);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		try {
			ChartUtilities.saveChartAsPNG(new File("Output/AODVM/AODVM_Umgesetzte_Energie_ohne_RouteDiscovery.png"),
	...
	public void evaluateNetworkLivetimeStaticSendBehavior(int payloadSize) {
		...
		/*
		 * double sendTime_1200[][] = new double[2][networkWidth.length]; double
		 * sendTime_1200_IdleMode[][] = new double[2][networkWidth.length];
		 * double sendTime_1200_ReciveMode[][] = new
		 * double[2][networkWidth.length]; double
		 * sendTime_1200_TransmissionMode[][] = new
		 * double[2][networkWidth.length]; double
		 * sendTime_1200_WaitingForMediumAccesPermission[][] = new
		 * double[2][networkWidth.length]; double
		 * sendTime_1200_PercentageRREQMsg[][] = new
		 * double[2][networkWidth.length]; double
		 * sendTime_1200_PercentageRREPMsg[][] = new
		 * double[2][networkWidth.length]; double
		 * sendTime_1200_PercentagePayloadMsg[][] = new
		 * double[2][networkWidth.length];
		 */
		for (int i = 0; i < networkWidth.length; i++) {
			numberOfNodes[i] = Math.pow(networkWidth[i], 2);

			sendTime_10[0][i] = numberOfNodes[i];
			sendTime_10[1][i] = simulator.lifetimeAnalysisStaticSendBehavior(networkWidth[i], 1 * 60, payloadSize)
					/ 1000 / 60;
			...
			sendTime_60[1][i] = simulator.lifetimeAnalysisStaticSendBehavior(networkWidth[i], 5 * 60, payloadSize)
					/ 1000 / 60;
			...
			sendTime_600[1][i] = simulator.lifetimeAnalysisStaticSendBehavior(networkWidth[i], 10 * 60, payloadSize)
					/ 1000 / 60;
			...
			/*
			 * sendTime_1200[0][i] = numberOfNodes[i]; sendTime_1200[1][i] =
			 * simulator.lifetimeAnalysisStaticSendBehavior(networkWidth[i],
			 * 20*60, payloadSize) / 1000 / 60; sendTime_1200_IdleMode[0][i] =
			 * numberOfNodes[i]; sendTime_1200_IdleMode[1][i] =
			 * simulator.getAverageTimeInIdleMode();
			 * sendTime_1200_ReciveMode[0][i] = numberOfNodes[i];
			 * sendTime_1200_ReciveMode[1][i] =
			 * simulator.getAverageTimeInReciveMode();
			 * sendTime_1200_TransmissionMode[0][i] = numberOfNodes[i];
			 * sendTime_1200_TransmissionMode[1][i] =
			 * simulator.getAverageTimeInTransmissionMode();
			 * sendTime_1200_WaitingForMediumAccesPermission[0][i] =
			 * numberOfNodes[i];
			 * sendTime_1200_WaitingForMediumAccesPermission[1][i] = simulator
			 * .getAverageTimeWaitingForMediumAccesPermission();
			 * sendTime_1200_PercentageRREQMsg[0][i] = numberOfNodes[i];
			 * sendTime_1200_PercentageRREQMsg[1][i] =
			 * simulator.getPercentageTransmittedRREQMsg();
			 * sendTime_1200_PercentageRREPMsg[0][i] = numberOfNodes[i];
			 * sendTime_1200_PercentageRREPMsg[1][i] =
			 * simulator.getPercentageTransmittedRREPMsg();
			 * sendTime_1200_PercentagePayloadMsg[0][i] = numberOfNodes[i];
			 * sendTime_1200_PercentagePayloadMsg[1][i] =
			 * simulator.getPercentageTransmittedPayloadMsg();
			 * System.out.println( "20m Simulation for " + numberOfNodes[i] +
		...
		String filename = "Output/AODVM/AODVM_Lebenszeitanalyse_" + payloadSize + "Byte.png";
		try {
		...
		// dataset2.addSeries("Knoten Sendet alle 20 m",
		// sendTime_1200_TransmissionMode);
		...
		filename = "Output/AODVM/AODVM_Lebenszeitanalyse_prozentualeZeit_Sendemodus_" + payloadSize + "Byte.png";
		try {
		...
		filename = "Output/AODVM/AODVM_Lebenszeitanalyse_prozentualeZeit_IdleModus_" + payloadSize + "Byte.png";
		try {
			ChartUtilities.saveChartAsPNG(new File(filename), chart2, CHART_WIDTH, CHART_HIGHT);
		} catch (IOException e) {
			// TODO Auto-generated catch block
		...
		// dataset2.addSeries("Knoten Sendet alle 20 m",
		// sendTime_1200_WaitingForMediumAccesPermission);
		...
		filename = "Output/AODVM/AODVM_Lebenszeitanalyse_prozentualeZeit_WaitingForMediumAccesPermission_" + payloadSize
				+ "Byte.png";
		try {
			ChartUtilities.saveChartAsPNG(new File(filename), chart2, CHART_WIDTH, CHART_HIGHT);
		} catch (IOException e) {
		...
		filename = "Output/AODVM/AODVM_Lebenszeitanalyse_prozentualeZeit_ReciveModus_" + payloadSize + "Byte.png";
		try {
			ChartUtilities.saveChartAsPNG(new File(filename), chart2, CHART_WIDTH, CHART_HIGHT);
		} catch (IOException e) {
			// TODO Auto-generated catch block
		...
		for (int i = 0; i < networkWidth.length; i++) {
			...
			sendTime_10_PercentageProtocolMsg[0][i] = sendTime_10_PercentageRREPMsg[1][i]
					+ sendTime_10_PercentageRREQMsg[1][i];
			sendTime_60_PercentageProtocolMsg[0][i] = numberOfNodes[i];
			sendTime_60_PercentageProtocolMsg[0][i] = sendTime_60_PercentageRREPMsg[1][i]
					+ sendTime_60_PercentageRREQMsg[1][i];
			sendTime_600_PercentageProtocolMsg[0][i] = numberOfNodes[i];
			sendTime_600_PercentageProtocolMsg[0][i] = sendTime_600_PercentageRREPMsg[1][i]
					+ sendTime_600_PercentageRREQMsg[1][i];
		}

		dataset2 = new DefaultXYDataset();
		// dataset2.addSeries("Knoten Sendet alle 10 s (Routinginformationen)",
		// sendTime_10_PercentageProtocolMsg);
		dataset2.addSeries("Knoten Sendet alle 10 s (Nachrichten)", sendTime_10_PercentagePayloadMsg);
		// dataset2.addSeries("Knoten Sendet alle 60 s (Routinginformationen)",
		// sendTime_60_PercentageProtocolMsg);
		dataset2.addSeries("Knoten Sendet alle 60 s (Nachrichten)", sendTime_60_PercentagePayloadMsg);
		// dataset2.addSeries("Knoten Sendet alle 600 s (Routinginformationen)",
		// sendTime_600_PercentageProtocolMsg);
		dataset2.addSeries("Knoten Sendet alle 10 min (Nachrichten)", sendTime_600_PercentagePayloadMsg);

		// dataset2.addSeries("Knoten Sendet alle 20 min (Nachrichten)",
		// sendTime_1200_PercentagePayloadMsg);
		...
		filename = "Output/AODVM/AODVM_Lebenszeitanalyse_prozentualeNachrichtenVerteilung" + payloadSize + "Byte.png";
		try {
			ChartUtilities.saveChartAsPNG(new File(filename), chart2, CHART_WIDTH, CHART_HIGHT);
		} catch (IOException e) {
			// TODO Auto-generated catch block
	...
	public void evaluateNetworkLivetimeStaticSendBehaviorOneDestination(int payloadSize) {
		...
		for (int i = 0; i < networkWidth.length; i++) {
			...
			/*
			 * sendTime_10_IdleMode[0][i] = numberOfNodes[i];
			 * sendTime_10_IdleMode[1][i] =
			 * simulator.getAverageTimeInIdleMode();
			 * sendTime_10_ReciveMode[0][i] = numberOfNodes[i];
			 * sendTime_10_ReciveMode[1][i] =
			 * simulator.getAverageTimeInReciveMode();
			 * sendTime_10_TransmissionMode[0][i] = numberOfNodes[i];
			 * sendTime_10_TransmissionMode[1][i] =
			 * simulator.getAverageTimeInTransmissionMode();
			 * sendTime_10_WaitingForMediumAccesPermission[0][i] =
			 * numberOfNodes[i];
			 * sendTime_10_WaitingForMediumAccesPermission[1][i] = simulator
			 * .getAverageTimeWaitingForMediumAccesPermission();
			 * sendTime_10_PercentageRREQMsg[0][i] = numberOfNodes[i];
			 ...
			 * simulator.getPercentageTransmittedRREQMsg();
			 * sendTime_10_PercentageRREPMsg[0][i] = numberOfNodes[i];
			 * sendTime_10_PercentageRREPMsg[1][i] =
			 * simulator.getPercentageTransmittedRREPMsg();
			 * sendTime_10_PercentagePayloadMsg[0][i] = numberOfNodes[i];
			 * sendTime_10_PercentagePayloadMsg[1][i] =
			 * simulator.getPercentageTransmittedPayloadMsg();
			 */
			System.out.println(
					"10s Simulation for " + numberOfNodes[i] + " nodes completed. Ausführungszeit des Netzwerks: "
			...
			/*
			 * sendTime_60_IdleMode[0][i] = numberOfNodes[i];
			 * sendTime_60_IdleMode[1][i] =
			 * simulator.getAverageTimeInIdleMode();
			 * sendTime_60_ReciveMode[0][i] = numberOfNodes[i];
			 * sendTime_60_ReciveMode[1][i] =
			 * simulator.getAverageTimeInReciveMode();
			 * sendTime_60_TransmissionMode[0][i] = numberOfNodes[i];
			 * sendTime_60_TransmissionMode[1][i] =
			 * simulator.getAverageTimeInTransmissionMode();
			 * sendTime_60_WaitingForMediumAccesPermission[0][i] =
			 * numberOfNodes[i];
			 * sendTime_60_WaitingForMediumAccesPermission[1][i] = simulator
			 * .getAverageTimeWaitingForMediumAccesPermission();
			 * sendTime_60_PercentageRREQMsg[0][i] = numberOfNodes[i];
			 ...
			 * simulator.getPercentageTransmittedRREQMsg();
			 * sendTime_60_PercentageRREPMsg[0][i] = numberOfNodes[i];
			 * sendTime_60_PercentageRREPMsg[1][i] =
			 * simulator.getPercentageTransmittedRREPMsg();
			 * sendTime_60_PercentagePayloadMsg[0][i] = numberOfNodes[i];
			 * sendTime_60_PercentagePayloadMsg[1][i] =
			 * simulator.getPercentageTransmittedPayloadMsg();
			 */
			System.out.println(
					"60s Simulation for " + numberOfNodes[i] + " nodes completed. Ausführungszeit des Netzwerks: "
			...
			/*
			 * sendTime_600_IdleMode[0][i] = numberOfNodes[i];
			 * sendTime_600_IdleMode[1][i] =
			 * simulator.getAverageTimeInIdleMode();
			 * sendTime_600_ReciveMode[0][i] = numberOfNodes[i];
			 * sendTime_600_ReciveMode[1][i] =
			 * simulator.getAverageTimeInReciveMode();
			 * sendTime_600_TransmissionMode[0][i] = numberOfNodes[i];
			 * sendTime_600_TransmissionMode[1][i] =
			 * simulator.getAverageTimeInTransmissionMode();
			 * sendTime_600_WaitingForMediumAccesPermission[0][i] =
			 * numberOfNodes[i];
			 * sendTime_600_WaitingForMediumAccesPermission[1][i] = simulator
			 * .getAverageTimeWaitingForMediumAccesPermission();
			 * sendTime_600_PercentageRREQMsg[0][i] = numberOfNodes[i];
			 ...
			 * simulator.getPercentageTransmittedRREQMsg();
			 * sendTime_600_PercentageRREPMsg[0][i] = numberOfNodes[i];
			 * sendTime_600_PercentageRREPMsg[1][i] =
			 * simulator.getPercentageTransmittedRREPMsg();
			 * sendTime_600_PercentagePayloadMsg[0][i] = numberOfNodes[i];
			 * sendTime_600_PercentagePayloadMsg[1][i] =
			 * simulator.getPercentageTransmittedPayloadMsg();
			 */
			System.out.println(
					"10m Simulation for " + numberOfNodes[i] + " nodes completed. Ausführungszeit des Netzwerks: "
		...
		String filename = "Output/AODVM/AODVM_Lebenszeitanalyse_OneDestination_" + payloadSize + "Byte.png";
		try {
			ChartUtilities.saveChartAsPNG(new File(filename), chart, CHART_WIDTH, CHART_HIGHT);
		} catch (IOException e) {
			// TODO Auto-generated catch block
		...
		/*
		 * // Average time in transmission mode DefaultXYDataset dataset2 = new
		 * DefaultXYDataset(); dataset2.addSeries("Knoten Sendet alle 10 s",
		 * sendTime_10_TransmissionMode); dataset2.addSeries(
		 * "Knoten Sendet alle 60 s", sendTime_60_TransmissionMode);
		 * dataset2.addSeries("Knoten Sendet alle 10 m",
		 * sendTime_600_TransmissionMode); //dataset2.addSeries(
		 * "Knoten Sendet alle 20 m", sendTime_1200_TransmissionMode);
		 * 
		 * NumberAxis xAxis2 = new NumberAxis("Anzahl Knoten im Netzwerk");
		 * NumberAxis yAxis2 = new NumberAxis("Knoten im Sendemodus [%]");
		 * XYPlot plot2 = new XYPlot(dataset2, xAxis2, yAxis2, line);
		 ...
		 * 
		 * filename =
		 * "Output/AODV_RFC/AODV_Lebenszeitanalyse_prozentualeZeit_Sendemodus_"
		 * + payloadSize + "Byte.png"; try { ChartUtilities.saveChartAsPNG(new
		 * File(filename), chart2, CHART_WIDTH, CHART_HIGHT); } catch
		 * (IOException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); }
		 * 
		 * // Average time in idle mode dataset2 = new DefaultXYDataset();
		 * dataset2.addSeries("Knoten Sendet alle 10 s", sendTime_10_IdleMode);
		 * dataset2.addSeries("Knoten Sendet alle 60 s", sendTime_60_IdleMode);
		 * dataset2.addSeries("Knoten Sendet alle 10 m", sendTime_600_IdleMode);
		 * //dataset2.addSeries("Knoten Sendet alle 20 m",
		 * sendTime_1200_IdleMode);
		 * 
		 * xAxis2 = new NumberAxis("Anzahl Knoten im Netzwerk"); yAxis2 = new
		 * NumberAxis("Knoten im Idle-Modus [%]"); plot2 = new XYPlot(dataset2,
		 * xAxis2, yAxis2, line);
		 * 
		 * chart2 = new JFreeChart(plot2);
		 * 
		 ...
		 * 
		 * filename =
		 * "Output/AODV_RFC/AODV_Lebenszeitanalyse_prozentualeZeit_IdleModus_" +
		 * payloadSize + "Byte.png"; try { ChartUtilities.saveChartAsPNG(new
		 * File(filename), chart2, CHART_WIDTH, CHART_HIGHT); } catch
		 * (IOException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); }
		 * 
		 * // Average time Waiting For MediumAccesPermission dataset2 = new
		 * DefaultXYDataset(); dataset2.addSeries("Knoten Sendet alle 10 s",
		 * sendTime_10_WaitingForMediumAccesPermission); dataset2.addSeries(
		 * "Knoten Sendet alle 60 s",
		 * sendTime_60_WaitingForMediumAccesPermission); dataset2.addSeries(
		 * "Knoten Sendet alle 10 m",
		 * sendTime_600_WaitingForMediumAccesPermission); //dataset2.addSeries(
		 * "Knoten Sendet alle 20 m",
		 * sendTime_1200_WaitingForMediumAccesPermission);
		 * 
		 * xAxis2 = new NumberAxis("Anzahl Knoten im Netzwerk"); yAxis2 = new
		 * NumberAxis("Knoten wartet auf Medium [%]"); plot2 = new
		 * XYPlot(dataset2, xAxis2, yAxis2, line);
		 * 
		 * chart2 = new JFreeChart(plot2);
		 * 
		 * chart2.getPlot().setBackgroundPaint(Color.WHITE);
		 ...
		 * 
		 * filename =
		 * "Output/AODV_RFC/AODV_Lebenszeitanalyse_prozentualeZeit_WaitingForMediumAccesPermission_"
		 * + payloadSize + "Byte.png"; try { ChartUtilities.saveChartAsPNG(new
		 * File(filename), chart2, CHART_WIDTH, CHART_HIGHT); } catch
		 * (IOException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); }
		 * 
		 * // Average time in recive mode dataset2 = new DefaultXYDataset();
		 * dataset2.addSeries("Knoten Sendet alle 10 s",
		 * sendTime_10_ReciveMode); dataset2.addSeries("Knoten Sendet alle 60 s"
		 * , sendTime_60_ReciveMode); dataset2.addSeries(
		 * "Knoten Sendet alle 10 m", sendTime_600_ReciveMode);
		 * //dataset2.addSeries("Knoten Sendet alle 20 m",
		 * sendTime_1200_ReciveMode);
		 * 
		 * xAxis2 = new NumberAxis("Anzahl Knoten im Netzwerk"); yAxis2 = new
		 * NumberAxis("Knoten im Empfangsmodus [%]"); plot2 = new
		 * XYPlot(dataset2, xAxis2, yAxis2, line);
		 * 
		 * chart2 = new JFreeChart(plot2);
		 * 
		 ...
		 * 
		 * filename =
		 * "Output/AODV_RFC/AODV_Lebenszeitanalyse_prozentualeZeit_ReciveModus_"
		 * + payloadSize + "Byte.png"; try { ChartUtilities.saveChartAsPNG(new
		 * File(filename), chart2, CHART_WIDTH, CHART_HIGHT); } catch
		 * (IOException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); }
		 ...
		 * // Comparison of data packages
		 * 
		 * double sendTime_10_PercentageProtocolMsg[][] = new
		 * double[2][networkWidth.length]; double
		 * sendTime_60_PercentageProtocolMsg[][] = new
		 * double[2][networkWidth.length]; double
		 * sendTime_600_PercentageProtocolMsg[][] = new
		 * double[2][networkWidth.length]; for(int i=0; i<networkWidth.length;
		 * i++){ sendTime_10_PercentageProtocolMsg[0][i] = numberOfNodes[i];
		 * sendTime_10_PercentageProtocolMsg[0][i] =
		 * sendTime_10_PercentageRREPMsg[1][i] +
		 * sendTime_10_PercentageRREQMsg[1][i];
		 * sendTime_60_PercentageProtocolMsg[0][i] = numberOfNodes[i];
		 * sendTime_60_PercentageProtocolMsg[0][i] =
		 * sendTime_60_PercentageRREPMsg[1][i] +
		 * sendTime_60_PercentageRREQMsg[1][i];
		 * sendTime_600_PercentageProtocolMsg[0][i] = numberOfNodes[i];
		 * sendTime_600_PercentageProtocolMsg[0][i] =
		 * sendTime_600_PercentageRREPMsg[1][i] +
		 * sendTime_600_PercentageRREQMsg[1][i]; }
		 * 
		 * dataset2 = new DefaultXYDataset(); //dataset2.addSeries(
		 * "Knoten Sendet alle 10 s (Routinginformationen)",
		 * sendTime_10_PercentageProtocolMsg); dataset2.addSeries(
		 * "Knoten Sendet alle 10 s (Nachrichten)",
		 * sendTime_10_PercentagePayloadMsg); //dataset2.addSeries(
		 * "Knoten Sendet alle 60 s (Routinginformationen)",
		 * sendTime_60_PercentageProtocolMsg); dataset2.addSeries(
		 * "Knoten Sendet alle 60 s (Nachrichten)",
		 * sendTime_60_PercentagePayloadMsg); //dataset2.addSeries(
		 * "Knoten Sendet alle 600 s (Routinginformationen)",
		 * sendTime_600_PercentageProtocolMsg); dataset2.addSeries(
		 * "Knoten Sendet alle 10 min (Nachrichten)",
		 * sendTime_600_PercentagePayloadMsg);
		 * 
		 * //dataset2.addSeries("Knoten Sendet alle 20 min (Nachrichten)",
		 * sendTime_1200_PercentagePayloadMsg);
		 * 
		 * 
		 * xAxis2 = new NumberAxis("Anzahl Knoten im Netzwerk"); yAxis2 = new
		 * NumberAxis("Gesendete Nachrichten [%]"); plot2 = new XYPlot(dataset2,
		 * xAxis2, yAxis2, line);
		 * 
		 * chart2 = new JFreeChart(plot2);
		 * 
		 ...
		 * 
		 * filename =
		 * "Output/AODV_RFC/AODV_Lebenszeitanalyse_prozentualeNachrichtenVerteilung"
		 * + payloadSize + "Byte.png"; try { ChartUtilities.saveChartAsPNG(new
		 * File(filename), chart2, CHART_WIDTH, CHART_HIGHT); } catch
		 * (IOException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); }
	...
	public void evaluateNetworkLivetimeStaticSendBehaviorChangingDestination(int payloadSize) {
		...
		for (int i = 0; i < networkWidth.length; i++) {
			...
			sendTime_10[0][i] = numberOfNodes[i];
			sendTime_10[1][i] = simulator.lifetimeAnalysisStaticSendBehaviorChangingDestination(networkWidth[i], 1 * 60,
					payloadSize) / 1000 / 60;
			sendTime_10_IdleMode[0][i] = numberOfNodes[i];
			sendTime_10_IdleMode[1][i] = simulator.getAverageTimeInIdleMode();
			sendTime_10_ReciveMode[0][i] = numberOfNodes[i];
			...
			sendTime_60[0][i] = numberOfNodes[i];
			sendTime_60[1][i] = simulator.lifetimeAnalysisStaticSendBehaviorChangingDestination(networkWidth[i], 5 * 60,
					payloadSize) / 1000 / 60;
			sendTime_60_IdleMode[0][i] = numberOfNodes[i];
			sendTime_60_IdleMode[1][i] = simulator.getAverageTimeInIdleMode();
			sendTime_60_ReciveMode[0][i] = numberOfNodes[i];
			...
			sendTime_600[0][i] = numberOfNodes[i];
			sendTime_600[1][i] = simulator.lifetimeAnalysisStaticSendBehaviorChangingDestination(networkWidth[i],
					10 * 60, payloadSize) / 1000 / 60;
			sendTime_600_IdleMode[0][i] = numberOfNodes[i];
			sendTime_600_IdleMode[1][i] = simulator.getAverageTimeInIdleMode();
			sendTime_600_ReciveMode[0][i] = numberOfNodes[i];
			sendTime_600_ReciveMode[1][i] = simulator.getAverageTimeInReciveMode();
			...
			/*
			 * sendTime_1200[0][i] = numberOfNodes[i]; sendTime_1200[1][i] =
			 * simulator.lifetimeAnalysisStaticSendBehaviorChangingDestination(
			 * networkWidth[i], 20*60, payloadSize) / 1000 / 60;
			 * sendTime_1200_IdleMode[0][i] = numberOfNodes[i];
			 * sendTime_1200_IdleMode[1][i] =
			 * simulator.getAverageTimeInIdleMode();
			 * sendTime_1200_ReciveMode[0][i] = numberOfNodes[i];
			 * sendTime_1200_ReciveMode[1][i] =
			 * simulator.getAverageTimeInReciveMode();
		...
		filename = "Output/AODVM/AODVM_Lebenszeitanalyse_prozentualeZeit_Sendemodus_WechselndeZielknoten_" + payloadSize
				+ "Byte.png";
		try {
			ChartUtilities.saveChartAsPNG(new File(filename), chart2, CHART_WIDTH, CHART_HIGHT);
		} catch (IOException e) {
		...
		filename = "Output/AODVM/AODVM_Lebenszeitanalyse_prozentualeZeit_IdleModus_WechselndeZielknoten_" + payloadSize
				+ "Byte.png";
		try {
			ChartUtilities.saveChartAsPNG(new File(filename), chart2, CHART_WIDTH, CHART_HIGHT);
		} catch (IOException e) {
		...
		filename = "Output/AODV_RFC/AODV_Lebenszeitanalyse_prozentualeZeit_ReciveModus_WechselndeZielknoten_"
				+ payloadSize + "Byte.png";
		try {
			ChartUtilities.saveChartAsPNG(new File(filename), chart2, CHART_WIDTH, CHART_HIGHT);
		} catch (IOException e) {
		...
		for (int i = 0; i < networkWidth.length; i++) {
			sendTime_10_PercentageProtocolMsg[0][i] += numberOfNodes[i];
			sendTime_10_PercentageProtocolMsg[1][i] += sendTime_10_PercentageRREPMsg[1][i]
					+ sendTime_10_PercentageRREQMsg[1][i];
			sendTime_60_PercentageProtocolMsg[0][i] += numberOfNodes[i];
			sendTime_60_PercentageProtocolMsg[1][i] += sendTime_60_PercentageRREPMsg[1][i]
					+ sendTime_60_PercentageRREQMsg[1][i];
			sendTime_600_PercentageProtocolMsg[0][i] += numberOfNodes[i];
			sendTime_600_PercentageProtocolMsg[1][i] += sendTime_600_PercentageRREPMsg[1][i]
					+ sendTime_600_PercentageRREQMsg[1][i];
		}
		...
		filename = "Output/AODV_RFC/AODV_Lebenszeitanalyse_prozentualeNachrichtenVerteilung_WechselndeZielknoten_"
				+ payloadSize + "Byte.png";
		try {
			ChartUtilities.saveChartAsPNG(new File(filename), chart2, CHART_WIDTH, CHART_HIGHT);
		} catch (IOException e) {
