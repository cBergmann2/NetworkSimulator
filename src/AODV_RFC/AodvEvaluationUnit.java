...
public class AodvEvaluationUnit extends EvaluationUnit {
	...
	public void evaluateSpeedAnalysis() {
		...
		for (int i = 0; i < networkWidth.length; i++) {
			...
			transmissionTime_min[1][i] = aodvSimulator.speedAnalysis(networkWidth[i], 0, 1);
			...
			transmissionTime_med[1][i] = aodvSimulator.speedAnalysis(networkWidth[i], 0, (networkWidth[i] / 2) * networkWidth[i] + networkWidth[i] / 2);
	...
	public void evaluateNetworkLivetimeStaticSendBehavior(int payloadSize) {
		...
		for(int i=0; i<networkWidth.length; i++){
			...
			sendTime_10[1][i] = aodvSimulator.lifetimeAnalysisStaticSendBehavior(networkWidth[i], 10, payloadSize) /1000 / 60;
			sendTime_10_TransmissionMode[0][i] = numberOfNodes[i];
			sendTime_10_TransmissionMode[1][i] = aodvSimulator.getAverageTimeInTransmissionMode();
			System.out.println("10s Simulation for " + numberOfNodes[i] + " nodes completed. Ausführungszeit des Netzwerks: " + aodvSimulator.getNetworkLifetime() /1000 / 60 + " min" );
		
			sendTime_60[0][i] = numberOfNodes[i];
			sendTime_60[1][i] = aodvSimulator.lifetimeAnalysisStaticSendBehavior(networkWidth[i], 60, payloadSize) /1000 / 60;
			sendTime_60_TransmissionMode[0][i] = numberOfNodes[i];
			sendTime_60_TransmissionMode[1][i] = aodvSimulator.getAverageTimeInTransmissionMode();
			System.out.println("60s Simulation for " + numberOfNodes[i] + " nodes completed. Ausführungszeit des Netzwerks: " + aodvSimulator.getNetworkLifetime() /1000 / 60 + " min" );
		
			sendTime_600[0][i] = numberOfNodes[i];
			sendTime_600[1][i] = aodvSimulator.lifetimeAnalysisStaticSendBehavior(networkWidth[i], 600, payloadSize) /1000 / 60;
			sendTime_600_TransmissionMode[0][i] = numberOfNodes[i];
			sendTime_600_TransmissionMode[1][i] = aodvSimulator.getAverageTimeInTransmissionMode();
			System.out.println("10m Simulation for " + numberOfNodes[i] + " nodes completed. Ausführungszeit des Netzwerks: " + aodvSimulator.getNetworkLifetime() /1000 / 60 + " min" );
		...
		String filename = "Output/AODV_RFC/AODV_Lebenszeit_statisch_" + payloadSize + "Byte.png"; 
		try {
		...
		filename = "Output/AODV_RFC/AODV_Lebenszeit_statisch_prozentualeZeit_Sendemodus_" + payloadSize + "Byte.png"; 
		try {
